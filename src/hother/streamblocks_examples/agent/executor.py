...
class ToolExecutor:
    ...
    def register_function(
        ...
        *,
        ...
        """Register a function and return its ToolDefinition.

        This is a non-decorator version of register() for list-based tool registration.
        ...
        Example:
            ...
            tool_def = executor.register_function(search)
        """
        # Call the existing register logic
        self.register(func, name=name, description=description, timeout=timeout, retries=retries)
        # Return the created ToolDefinition
        tool_name = name or func.__name__
        return self._tools[tool_name]

    def register_many(
        self,
        funcs: list[Callable[..., Any] | ToolDefinition],
    ) -> list[ToolDefinition]:
        """Register multiple tools at once.

        Accepts a list of either:
        - Callable functions (will be introspected and registered)
        - ToolDefinition objects (will be registered directly)

        Args:
            funcs: List of callables or ToolDefinition objects
        ...
        Example:
            ...
            def order(product_id: str, quantity: int) -> dict:
                '''Create an order.'''
                return {}

            tools = executor.register_many([search, order])
        """
        tools = []
        for func in funcs:
            if isinstance(func, ToolDefinition):
                ...
                tools.append(self.register_function(func))
        return tools
