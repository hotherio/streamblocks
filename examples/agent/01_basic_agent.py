...
async def run_streamblocks_speculative(task: str) -> tuple[str, int, float]:
    """Run the StreamBlocks speculative agent.

    Returns:
        (answer, tools_called, elapsed_time)
    """
    print("=" * 70)
    print("STREAMBLOCKS SPECULATIVE AGENT")
    print("(Tools execute IN PARALLEL with LLM streaming)")
    print("=" * 70)
    print()
    ...
    async for event in agent.run_stream(task):
        if isinstance(event, TextDeltaEvent):
            # Print streamed text character by character
            ...
            # Print truncated parameters for readability
            params_str = str(event.parameters)
            print(f"  Parameters: {params_str}")

        elif isinstance(event, ToolStartedEvent):
            print(f"[Tool {event.tool_name} started in background...]")
            ...
            if event.injected:
                print("[Result injected mid-stream]")

        elif isinstance(event, StreamCancelledEvent):
            print(f"\n[Stream cancelled: {event.reason}]")
            turn += 1
            print(f"\n--- Turn {turn} ---\n")
    ...
    print("TOKEN METRICS")
    print("-" * 50)
    print(f"LLM API calls: {len(llm_call_metrics)}")
    ...
    return final_answer, tools_called, elapsed
...
async def run_pydantic_ai(task: str) -> tuple[str, int, float]:
    ...
    return answer, tools_called, elapsed
...
async def main() -> None:
    ...
    print()
    print("StreamBlocks vs Pydantic AI - Speculative Agent Comparison")
    print("=" * 70)
    print()
    ...
    _sb_answer, sb_tools, sb_time = await run_streamblocks_speculative(task)

    # Run Pydantic AI agent
    _pai_answer, pai_tools, pai_time = await run_pydantic_ai(task)
    ...
    print("=" * 70)
    print()
    print(f"{'Metric':<25} {'StreamBlocks':<20} {'Pydantic AI':<20}")
    print("-" * 65)
    print(f"{'Tools called':<25} {sb_tools:<20} {pai_tools:<20}")
    print(f"{'Time (seconds)':<25} {sb_time:<20.2f} {pai_time:<20.2f}")
    print()
    print("Note: StreamBlocks uses SPECULATIVE continuation:")
    print("      - Tools run in parallel with LLM streaming")
    print("      - Results injected mid-stream when ready")
    print()
    print("      Pydantic AI uses native function calling (API-level)")
    print()
    print("With slow tools (1.2-2.0s), speculative execution can save time")
    print("by running tools while the LLM continues generating.")
    print()
    print("See 02_sequential_agent.py for the parallel-after-LLM comparison.")
    print("See 03_batched_speculative.py for the batched approach.")
