...
async def run_streamblocks_sequential(task: str) -> tuple[str, int, float]:
    ...
    """
    print("=" * 70)
    print("STREAMBLOCKS SEQUENTIAL AGENT")
    print("(Tools execute IN PARALLEL after LLM finishes)")
    print("=" * 70)
    print()
...
Returns:
    ...
    async for event in stream.run(task):
        if isinstance(event, TextDeltaEvent):
            ...
            print(f"\n[Tool call queued: {event.tool_name}]")
            ...
            print(f"\n[Tool executed ({status}): {result_str}]")
            turn += 1
            print(f"\n[Turn {turn}]")
...
async def run_pydantic_ai(task: str) -> tuple[str, int, float]:
    ...
    print("RESULTS")
    print("-" * 50)
    print(f"Final Answer: {answer[:200]}..." if len(answer) > 200 else f"Final Answer: {answer}")
    print(f"Tools called: {tools_called}")
    print(f"Time: {elapsed:.2f}s")
...
async def main() -> None:
    ...
    print()
    print("StreamBlocks vs Pydantic AI - Sequential Agent Comparison")
    print("=" * 70)
    print()
    ...
    _sb_answer, sb_tools, sb_time = await run_streamblocks_sequential(task)
    ...
    print()
    print("Note: Both use PARALLEL-AFTER-LLM pattern:")
    print("      - LLM generates complete response")
    print("      - ALL tools run in parallel after LLM stops")
    print()
    print("      StreamBlocks: Block-based tool calling (text parsing)")
    print("      Pydantic AI: Native function calling (API-level)")
    print()
    print("See 01_basic_agent.py for the SPECULATIVE approach (faster with slow tools).")
